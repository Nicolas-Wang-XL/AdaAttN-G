...
class artvggAdaAttNModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        content_image_encoder = torchvision.models.vgg16(pretrained=True).features
        ...
        enc_3 = nn.DataParallel(nn.Sequential(*enc_layers[9:16]).to(opt.gpu_ids[0]), opt.gpu_ids)
        enc_4 = nn.DataParallel(nn.Sequential(*enc_layers[16:23]).to(opt.gpu_ids[0]), opt.gpu_ids)
        enc_5 = nn.DataParallel(nn.Sequential(*enc_layers[23:30]).to(opt.gpu_ids[0]), opt.gpu_ids)
